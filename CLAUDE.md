...
- **Config message actions** — Content↔background CRUD: `getConfigs` (list all), `getConfig` (single by id, auto-defaults `apiMode`/`customHeaders`/`customBodyFields` for old data), `setActiveConfig` (sets `selected: true` on one, false on others), `addConfig` (pushes new item with `selected: true`), `editConfig` (updates name/url/model/apiKey/apiMode/customHeaders/customBodyFields), `deleteConfig` (removes, shifts selection if deleted was selected).
