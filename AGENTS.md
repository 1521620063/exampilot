...
- **Custom prompt storage** — `customPrompt` is a separate string key in `chrome.storage.local` (not in `configList`). Message handlers: `getPrompt`, `setPrompt`, and `resetPrompt`; `resetPrompt` persists and returns `DEFAULT_PROMPT` for the config-panel restore-default action.
