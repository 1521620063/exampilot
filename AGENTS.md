...
| `content/bundle/content-bundle.js` | esbuild output (IIFE) created under `dist/chrome/`. Default build injects it programmatically; Full Access build auto-loads it via `content_scripts` |
...
- **Build output structure** — Default builds write to `dist/chrome/`; Full Access builds write to `dist/chrome-full/`. Both contain `manifest.json`, `icons/`, `permission/`, and bundled content/background files. Load the desired dist directory in Chrome, not the project root.
