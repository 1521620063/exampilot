...
| 🕶️ 在 ⚙️ 中开启静默模式 | AI 返回答案与百分比坐标，页面创建透明命中区，鼠标悬停后按配置距离滚动 |
...
- 页面会为每个答案创建透明命中区；鼠标进入并短暂停留后，页面按“静默滚动距离”向下滚动
- 每次 AI 识别完成后也会自动按相同距离向下滚动一次
- “显示静默框”只控制是否画出红色调试框；关闭后命中区和悬浮滚动仍然有效
...
在 ⚙️ 设置页面的 **🎨 界面设置** 中可调节悬浮面板透明度、静默模式开关、静默滚动距离和静默调试框显示。界面偏好存储在 `chrome.storage.local`，仅影响本地显示与交互，不会发送给 AI API 或任何第三方服务。
