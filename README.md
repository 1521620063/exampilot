...
| 💾 在 ⚙️ → 配置迁移中导入/导出 | 将全部模型配置、提示词和透明度备份为 JSON，或在其他设备恢复 |
...
在 ⚙️ 设置页面的 **🎨 界面设置** 中可调节悬浮面板透明度。该偏好以 `uiOpacity` 存储在 `chrome.storage.local`，仅影响本地显示，不会发送给 AI API 或任何第三方服务。

### 💾 配置迁移

在 ⚙️ 设置页面的 **配置迁移** 中可以备份或恢复设置：

- **导出配置**会下载一个带日期的 `exampilot-settings-YYYY-MM-DD.json` 文件，包含全部模型配置（含 API Key）、当前选中项、自定义提示词和界面透明度
- **导入配置**会先校验备份格式、版本、HTTPS API 地址和接口模式，再替换当前浏览器中的全部 ExamPilot 配置、提示词与透明度
